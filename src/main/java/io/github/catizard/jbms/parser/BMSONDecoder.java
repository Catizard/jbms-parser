...
import io.github.catizard.jbms.parser.bmson.Note;
...
public class BMSONDecoder extends ChartDecoder {
	...
	public BMSModel decode(Path f) {
					...
					if (insideln) {
						...
						if (n.l > 0) {
							...
							if (tl.getNote(key) != null) {
								// レイヤーノート判定
								io.github.catizard.jbms.parser.Note en = tl.getNote(key);
								...
								if (existNote) {
									...
									// start.getTime());
									LongNote lnend = null;
									for (Entry<Note, LongNote> up : lnup.entrySet()) {
		...
		for (MineChannel sc : bmson.key_channels) {
			...
			final int length = sc.notes.length;
			for (int i = 0; i < length; i++) {
				final io.github.catizard.jbms.parser.bmson.MineNote n = sc.notes[i];
				TimeLine tl = getTimeLine(n.y, resolution);
